...
void shmem_BFS(const Graph &g, NodeID source, pvector<CountT> &path_counts,
    Bitmap &succ, vector<SlidingQueue<NodeID>::iterator> &depth_index,
    SlidingQueue<NodeID> &queue, Partition<NodeID> vp, long* PATH_LOCK, long long* pWrk, long* pSync) {
    ...
    for (NodeID q_iter = qp.start; q_iter < qp.end; q_iter++) {
      ...
      for (NodeID &v : g.out_neigh(u)) {
        ...
        if ((shmem_long_g(depths.begin()+lp_v, vp.recv(v)) == -1) &&
            (shmem_long_atomic_compare_swap(depths.begin()+lp_v, static_cast<long>(-1), depth, vp.recv(v)) == depth)) {
        //if ((depths[v] == -1) &&
        //    (compare_and_swap(depths[v], static_cast<NodeID>(-1), depth))) {          // is the depths[v] == -1 comparison just for short circuit evaluation?
          printf("PE %d pushing back %lu\n", vp.pe, v);
          lqueue.push_back(v);
        }
        if (shmem_long_g(depths.begin()+lp_v, vp.recv(v)) == depth) {
          printf("PE %d setting bit %lu\n", vp.pe, v);
          ...
          printf("lp_u = %d, own_u = %d\n", vp.local_pos(u), vp.recv(u));
          printf("pc[u] = %d\n", shmem_double_g(path_counts.begin()+vp.local_pos(u), vp.recv(u)));
          ...
          printf("PE %d combining path counts total = %d u = %d, v = %d\n", vp.pe, pc_u, pc_v, );
  ...
  shmem_barrier_all();
  succ.merge(pWrk, pSync);
  depth_index.push_back(queue.begin());
}


pvector<ScoreT> Brandes(const Graph &g, SourcePicker<Graph> &sp,
                        NodeID num_iters, long* PATHLOCK, long* QLOCK, long long* pWrk, long* pSync) {
  Timer t;
  t.Start();
  Partition<NodeID> vp(g.num_nodes());
  pvector<ScoreT> scores(vp.max_width, 0, true);                // symmetric partitioned pvector
  pvector<CountT> path_counts(vp.max_width, 0, true);                   // symmetric partitioned pvector
  ...
  for (NodeID iter=0; iter < num_iters; iter++) {
    ...
    queue.reset();
    succ.reset();
    shmem_BFS(g, source, path_counts, succ, depth_index, queue, vp, PATHLOCK, pWrk, pSync);
    printf("Pe %d Path counts: ", vp.pe);
    for (NodeID n = vp.start; n < vp.end; n++)
      printf("%d ", path_counts[vp.local_pos(n)]);
    printf("\n");
...
int main(int argc, char* argv[]) {
...

  {
    ...
    auto BCBound =
      [&sp, &cli] (const Graph &g) { return Brandes(g, sp, cli.num_iters(), &PATHLOCK, &QLOCK, ll_pWrk, pSync); };

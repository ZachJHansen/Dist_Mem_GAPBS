...
void PRBenchmarkKernel(const CLApp &cli, const GraphT_ &g,
  ...
  shmem_out.open("pr_output.txt"); //open a file to perform read operation using file object
  ...
  std::remove("pr_output.txt");               // delete file
  g.PrintStats();
...
void BFSBenchmarkKernel(const CLApp &cli, const GraphT_ &g,
  ...
  master.clear();
  std::ifstream shmem_out;
  shmem_out.open("bfs_output.txt"); //open a file to perform read operation using file object
  if (shmem_out.is_open()){                     //checking whether the file is open
    std::string tp;
    while(getline(shmem_out, tp))               //read data from file object and put it into string.
  ...
  std::remove("bfs_output.txt");               // delete file
  g.PrintStats();
...
void CCBenchmarkKernel(const CLApp &cli, const GraphT_ &g,
  ...
  master.clear();
  std::ifstream shmem_out;
  shmem_out.open("cc_output.txt"); //open a file to perform read operation using file object
  if (shmem_out.is_open()){                     //checking whether the file is open
    std::string tp;
    while(getline(shmem_out, tp))               //read data from file object and put it into string.
        master.push_back(std::stoi(tp));        // bfs and connected components produce parent arrays (ints)
  ...
  std::remove("cc_output.txt");               // delete file
  g.PrintStats();
  double total_seconds = 0;
  Timer trial_timer;
  pvector<int> result(g.num_nodes());
...
void TCBenchmarkKernel(const CLApp &cli, const GraphT_ &g,
  ...
  master.clear();
  std::ifstream shmem_out;
  shmem_out.open("tc_output.txt"); //open a file to perform read operation using file object
  if (shmem_out.is_open()){   //checking whether the file is open
    std::string tp;
    while(getline(shmem_out, tp)){  //read data from file object and put it into string.
  ...
  std::remove("tc_output.txt");               // delete file
  g.PrintStats();
  double total_seconds = 0;
  Timer trial_timer;
  for (int iter=0; iter < cli.num_trials(); iter++) {
...
void SSSPBenchmarkKernel(const CLApp &cli, const GraphT_ &g,
  ...
  master.clear();
  std::ifstream shmem_out;
  shmem_out.open("sssp_output.txt"); //open a file to perform read operation using file object
  if (shmem_out.is_open()){                     //checking whether the file is open
    std::string tp;
    while(getline(shmem_out, tp))               //read data from file object and put it into string.
  ...
  std::remove("sssp_output.txt");               // delete file
  g.PrintStats();
  double total_seconds = 0;
  Timer trial_timer;
...
void BCBenchmarkKernel(const CLApp &cli, const GraphT_ &g,
  ...
  master.clear();
  std::ifstream shmem_out;
  shmem_out.open("bc_output.txt"); //open a file to perform read operation using file object
  if (shmem_out.is_open()){                     //checking whether the file is open
    std::string tp;
    while(getline(shmem_out, tp))               //read data from file object and put it into string.
  ...
  std::remove("bc_output.txt");               // delete file
  g.PrintStats();
  double total_seconds = 0;
  Timer trial_timer;
  pvector<float> result(g.num_nodes());

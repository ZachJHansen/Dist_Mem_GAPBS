...
pvector<ScoreT> Brandes(const Graph &g, SourcePicker<Graph> &sp,
  ...
  for (NodeID iter=0; iter < num_iters; iter++) {
    ...
    t.Stop();
    /*printf("Depth index: %p\n", (void*) depth_index[0]);
    printf("Q begin: %d\n", *(queue.begin()));
    for (int i = 0; i < g.num_nodes(); i++) 
      printf("%f ", path_counts[i]);  
    printf("\n");
    for (int i = 0; i < g.num_edges_directed(); i++)
      printf("%d ", succ.get_bit(i));
    printf("\n");*/
...
bool BCVerifier(const pvector<ScoreT> &orig_scores, const pvector<ScoreT> &scores) {
  bool flag = true;
  for (int i = 0; i < orig_scores.size(); i++) {
    if (!(orig_scores[i] <= scores[i] + 0.0000001 && orig_scores[i] >= scores[i] - 0.0000001)) {
      printf("Ours: %f | Theirs: %f | Diff: %f\n", scores[i], orig_scores[i], orig_scores[i] - scores[i]);
      flag = false;
    }
  }
  return flag;
...
int main(int argc, char* argv[]) {
  ...
  auto VerifierBound = [&vsp, &cli] (const pvector<ScoreT> &orig_scores,
                                     const pvector<ScoreT> &scores) {
    return BCVerifier(orig_scores, scores);
  };
  BCBenchmarkKernel(cli, g, BCBound, PrintTopScores, VerifierBound);
  return 0;
}

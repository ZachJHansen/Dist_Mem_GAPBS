...
/*  pvector<NodeID> comp(g.num_nodes());
  ...
  /*pvector<int> c = CCBound(g);
  ofstream orig_out;
  orig_out.open("/home/zach/projects/Dist_Mem_GAPBS/gapbs/gapbs/src/cc_output.txt", ios::app);
  for (auto lable : c)
    orig_out << lable << endl;
  orig_out.close();*/
  CCBenchmarkKernel(cli, g, CCBound, PrintCompStats, CCVerifier);
  return 0;
}

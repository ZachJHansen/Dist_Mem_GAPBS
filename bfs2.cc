...
int64_t SHMEM_TDStep(const Graph &g, pvector<NodeID> &parent, SlidingQueue<NodeID> &queue, 
  ...
  while (q_iter < q_end) {      // what if q size < npes?
    ...
    for (NodeID v : g.out_neigh(u)) {
      if (vp.begin <= v && v < vp.end) {                                                // The outgoing neighbor v of node u is in the local subset of the parent array
        shmem_set_lock(PLOCKS+vp.pe);                                                   // A PE can lock itself to avoid simultaneous remote accesses to nodes in the local subset
        curr_val = parent[vp.local_pos(v)];                                             // v is the absolute location in the complete parent array
        if (curr_val < 0) {
          parent[vp.local_pos(v)] = u;                                                  // Update local subset with u
          lqueue.push_back(v);
          *(scout_count) += -curr_val;
        }  
        shmem_clear_lock(PLOCKS+vp.pe);
      } else {                                                                          // v is in the parent array subset on a different PE
        shmem_set_lock(PLOCKS+vp.recv(v));                                             // Get exclusive access to the parent array on the PE responsible for v
        shmem_int_get(&curr_val, parent.begin()+vp.local_pos(v), 1, vp.recv(v));                // Update curr_val with v from foreign pe's parent array
        if (curr_val < 0) {
          shmem_int_atomic_swap(parent.begin()+vp.local_pos(v), u, vp.recv(v));                 // Update foregin pe with u
          lqueue.push_back(v);                                                          // The sliding queue and scouts get aggregated, so it shouldnt matter which PE updates them?
          *(scout_count) += -curr_val;
        }
        shmem_clear_lock(PLOCKS+vp.recv(v));                                           
      }
    }
    q_iter++;
  }
